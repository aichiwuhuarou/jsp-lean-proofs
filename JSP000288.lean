...
/-!
# JSP-000288: A Counterexample to Erdős Problem 346

Justin Sun Prize problem **JSP-000288** (Number theory; equivalent to
[erdosproblems.com/346](https://www.erdosproblems.com/346)) asks:

> Must ratios of consecutive terms in the specified minimal stably complete
> sequences converge to the golden ratio `φ = (1 + √5) / 2`?

In the terminology of the official record, a strictly increasing sequence
`A = (aₙ)` of positive integers is

* *stably complete* when `A \ B` is *complete* (every sufficiently large integer
  is a finite subset sum of `A \ B`) after the deletion of any finite
  subsequence `B`, and
* *minimal* when `A \ B` is *not* complete after the deletion of any infinite
  subsequence `B`.

Erdős and Graham asked whether every such sequence whose consecutive quotients
are bounded below by `1 + ε` must satisfy `aₙ₊₁ / aₙ → φ`.

This file formalises a counterexample, giving a negative answer (the official
record: "Solved by GPT Pro, prompted by Liam Price (counterexample)").  We
construct a sequence `A` enjoying both deletion properties, with
`aₙ₊₁ / aₙ ≥ 6/5` throughout, yet whose quotients have two distinct
subsequential limits `φ` and `φ + 1/4`; in particular `(aₙ₊₁ / aₙ)` does not
converge.  The packaged statements are `erdos346` and `jsp_000288`.
...
* Justin Sun Prize awards catalog, problem JSP-000288,
  `catalog-0201-0300.md` in `TheJustinSunPrize/awards`.
...
lemma graham_odd_term_sub_mem
  ...
  have hsumE :
    ...
    show (∑ i ∈ (Finset.range t).map e, x i) = _
    exact Finset.sum_map (Finset.range t) e x
...
lemma graham_even_term_sub_mem
  ...
  have hsumA :
    ...
    show (∑ i ∈ (Finset.range (2 * s)).map lowEmb, x i) = _
    exact Finset.sum_map (Finset.range (2 * s)) lowEmb x
  have hsumB :
      (∑ i ∈ B, x i) = ∑ k ∈ Finset.range (d + 1), x (2 * (h + k) - 1) := by
    show (∑ i ∈ (Finset.range (d + 1)).map highEmb, x i) = _
    exact Finset.sum_map (Finset.range (d + 1)) highEmb x
...
lemma graham_odd_term_add_mem
  ...
  have hsumE :
    ...
    show (∑ i ∈ (Finset.range d).map e, x i) = _
    exact Finset.sum_map (Finset.range d) e x
...
/-!
The literal answer to JSP-000288: minimal stably complete sequences whose
consecutive quotients are bounded below by `1 + ε` need **not** have quotients
converging to `φ` — the quotients of the sequence constructed above (which
satisfies all the hypotheses with `ε = 1/5`) do not converge at all.
-/
theorem jsp_000288 :
    ¬ ∀ a : ℕ → ℕ, StrictMono a → (∀ n, 0 < a n) →
      (∀ D : Set ℕ, D.Finite →
  ...
  intro h
  obtain ⟨a, hmono, hpos, hfin, hinf, hgrowth, _, hnc⟩ := erdos346
  refine hnc ⟨φ, h a hmono hpos hfin hinf ⟨(1 : ℝ) / 5, by norm_num, ?_⟩⟩
  intro n
  have hle := hgrowth n
  linarith

#print axioms Erdos346.erdos346
#print axioms Erdos346.jsp_000288
